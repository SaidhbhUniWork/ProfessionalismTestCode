...
def countdown(time, frame):
    if time == -1:
        ...
        l4 = tk.Label(frame, text="Time remaining: %d seconds" % time, anchor='w',
                      bg="lightcyan1")  # show with a label how much time is left
...
def search_user(user, password, check):
    ...
    result = find_record(my_conn, query)  # search database for correct ID number for deletion
...
var1 = IntVar()
checkBtn = tk.Checkbutton(LoginFrame, text="Accept terms and conditions", onvalue=1, offvalue=0, variable=var1,
                          bg="lightcyan1").grid(row=6, column=1, sticky=W)

link = tk.Label(LoginFrame, text="Policy Document Link", fg="blue", cursor="hand2", bg="lightcyan1")
link.grid(row=7, column=1)
link.bind("<Button-1>", callback)

lLogin = tk.Label(LoginFrame, text='Administrator Log In', font=('Helvetica', 12), width=30, anchor="c",
                  bg="lightcyan1")
...
def select_user(
        event):  # when user clicks on an entry in the data grid table the selected record is inputted into the 'add employee' boxes
    try:
...
def display_data():
    for index in Admin_asset_tree_view.get_children():  # loops through all records in the data grid. get_children() returns a list of all row's iid
        Admin_asset_tree_view.delete(
            index)  # iterate through all rows, passing the iid to the delete() method to clear the grid ready for population
...
def delete():
    ...
    if len(tid4.get("1.0",
                    END)) <= 1:  # if the length of the inputted data in the delete text box is less than 0 (box is empty)
        my_id3 = tid2.get('1.0',
                          END)  # take the ID value from the 'add ID' textbox which was populated by user selection
...
Admin_asset_tree_view = Treeview(Admin_asset_frame, columns=columns,
                                 show="headings")  # creates treeview widget, asset tree_view, in the asset_frame with specified column names
Admin_asset_tree_view.column("ID", width=20)
for col in columns[0:]:  # for all columns in the data
    Admin_asset_tree_view.column(col,
                                 width=120)  # set each column width, looking through column names from the above array
...
Admin_scrollbar = Scrollbar(Admin_asset_frame,
                            orient='vertical')  # creates a vertical scrollbar widget for the data grid
...
# ID Label for search
lid_e = tk.Label(label_frame_SearchGrid, text='Enter User ID: ', width=11, font=('bold', 10), anchor="e",
                 bg="lightcyan1")
...
# search user button
b1_e = tk.Button(label_frame_SearchGrid, text='Search User', width=10, bg='white',
                 command=lambda: search(tid_e.get('1.0', END)), font=('bold', 10))
...
columns = ['ID', 'Name', 'Surname', 'Asset']  # set column headings
asset_tree_view = Treeview(asset_frame, columns=columns,
                           show="headings")  # creates treeview widget, asset tree_view, in the asset_frame with specified column names
...
# search user button
b1 = tk.Button(label_frame_SearchGrid, text='Search User', width=10, bg='white',
               command=lambda: search(tid.get('1.0', END)), font=('bold', 10))
