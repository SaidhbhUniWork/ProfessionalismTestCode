...
def display_query_data(query):
    #for index in asset_tree_view.get_children():
    #    asset_tree_view.delete(index)
    #r_set = my_conn.execute(query)
    for row in my_conn.execute(query):
        messagebox.showinfo("Employee Details", f"ID: {row[0]}\nName: {row[1]}\nSurname: {row[2]}\nAsset: {row[3]}" )
...
def countdown(time):
    if time == -1:  # if the time reaches -1, terminate the program
        ...
        l4 = tk.Label(my_w_1, text="Time remaining: %d seconds" % time, anchor='w')  # show with a label how much time is left
...
def execute_query():
    ...
    param = optionsS.get()
    thisQuery = """SELECT * FROM User where """ + param + """ = '""" + query + """'"""

    #qcursor = my_conn.cursor()
    #result = None
    #qcursor.execute(thisQuery)
    #result = qcursor.fetchmany(1)
    #for row in result
    #my_cursor = my_conn.execute(thisQuery)
    #data_row = my_cursor.fetchone()
    #name = data_row[1]
    #id = data_row[0]
    #sname = data_row[2]
    #asset = data_row[3]
    #messagebox.showinfo("Employee Details", f"ID: {result[0]}\nName: {result[1]}\nSurname: {result[2]}\nAsset: {result[3]}" )

    display_query_data(thisQuery)
...
bq = tk.Button(my_w_1, text='Search Records', width=12, bg='light grey', command=execute_query, font = ('bold', 10))
bq.grid(row=3, column=11)
...
bExit.grid(row=14, column=0)

# Label for Query Search
lbl_sqlSearch = tk.Label(my_w_1, text='Search by:', width=15, anchor="e", font=('bold', 10))
lbl_sqlSearch.grid(row=1, column=10, sticky=W)

# Scroll box for query text input
optionsS = StringVar(my_w_1)
optionsS.set("Select Option")  # default value

optSearch = OptionMenu(my_w_1, optionsS, "ID", "Name", "Surname", "Asset")
optSearch.config(width = 10)
optSearch.grid(row=1, column=11)

query_search = StringVar()
#query_search.set("Select * from User") 
query_search_entry = tk.Entry(my_w_1, width=15, textvariable=query_search, bg='white')
query_search_entry.grid(row=2, column=11)
